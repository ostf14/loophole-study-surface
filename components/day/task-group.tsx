...
export function TaskGroup({ group, done, open, onToggle, children }: TaskGroupProps) {
  ...
  return (
    <Card className="flex flex-col">
      ...
      {/* Вертикальный воздух телу карточки. Строка задачи — компонент
          `Tandem_Plan_Item`, 32 в высоту с паддингом 4/20, и трогать её нельзя;
          но про то, как список сидит в контейнере, компонент не говорит ничего,
          это уже наша композиция. А сидел он впритык: первая строка начиналась
          сразу под разделителем шапки, последняя упиралась в нижнюю рамку, и
          текст оказывался в шести пикселях от края карточки — два на рамку,
          четыре на паддинг строки.

          Восемь сверху и снизу дают от разделителя до текста двенадцать.
          Паддинг вертикальный, поэтому разделители между строками остаются
          во всю ширину: список внутри карточки, а не столбик карточек. */}
      {open ? (
        <div className="flex flex-col border-t-[2px] border-soft-black py-2">{children}</div>
      ) : null}
    </Card>
  );
}
